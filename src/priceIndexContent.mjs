...
export const PRICE_INDEX = {
  en: {
    ...
    citiesHeading: 'Price per square metre by city',
    citiesSub: 'The coarser cut, and the one most people search for. Al Seeb is its own wilayat but sits inside Muscat Governorate, so treat the two Muscat rows together when you compare against another country.',
    citiesCol: 'Communities',
    ...
    citeHeading: 'Citing this index',
    citeIntro: 'This page is free to quote and free to link to. Please carry the sample size and the date with the figure, and say that it covers freehold stock open to foreign buyers rather than the whole Oman market.',
    citeLabel: 'Ready made citation',
    citeCopy: 'Copy citation',
    citeCopied: 'Copied',
    citeDataLabel: 'Machine readable version',
    citeDataNote: 'The same figures as JSON, regenerated with the page. Use it for charts, spreadsheets or research.',
  ...
  fa: {
    ...
    citiesHeading: 'قیمت هر مترمربع بر اساس شهر',
    citiesSub: 'برش کلی‌تر، و همانی که بیشتر مردم جست‌وجو می‌کنند. السیب ولایت جداگانه‌ای است اما داخل استانداری مسقط قرار دارد، پس هنگام مقایسه با کشوری دیگر دو ردیف مسقط را کنار هم ببینید.',
    citiesCol: 'مناطق',
    ...
    tocHeading: 'در این صفحه',
    citeHeading: 'استناد به این شاخص',
    citeIntro: 'نقل قول و لینک دادن به این صفحه آزاد است. لطفاً اندازه نمونه و تاریخ را همراه رقم بیاورید و بنویسید که این داده مربوط به املاک فری‌هولد قابل خرید برای اتباع خارجی است، نه کل بازار عمان.',
    citeLabel: 'متن آماده استناد',
    citeCopy: 'کپی متن استناد',
    citeCopied: 'کپی شد',
    citeDataLabel: 'نسخه ماشین‌خوان',
    citeDataNote: 'همان ارقام به صورت JSON، که همراه صفحه بازتولید می‌شود. برای نمودار، صفحه گسترده یا پژوهش از آن استفاده کنید.',
  ...
  ar: {
    ...
    citiesHeading: 'سعر المتر المربع حسب المدينة',
    citiesSub: 'التقسيم الأوسع، وهو ما يبحث عنه معظم الناس. السيب ولاية مستقلة لكنها ضمن محافظة مسقط، لذا اقرأ صفَّي مسقط معاً عند المقارنة بدولة أخرى.',
    citiesCol: 'المناطق',
    ...
    citeHeading: 'الاقتباس من هذا المؤشر',
    citeIntro: 'الاقتباس من هذه الصفحة والربط بها مجاني. يرجى ذكر حجم العينة والتاريخ مع الرقم، وتوضيح أنها تغطي مخزون التملّك الحر المتاح للمشترين الأجانب وليس سوق عُمان بأكمله.',
    citeLabel: 'صيغة اقتباس جاهزة',
    citeCopy: 'نسخ الاقتباس',
    citeCopied: 'تم النسخ',
    citeDataLabel: 'نسخة قابلة للقراءة آلياً',
    citeDataNote: 'الأرقام نفسها بصيغة JSON، تُحدَّث مع الصفحة. استخدمها للرسوم البيانية أو الجداول أو البحث.',
  ...
  ru: {
    ...
    citiesHeading: 'Цена квадратного метра по городам',
    citiesSub: 'Более крупный срез, и именно его чаще всего ищут. Эс-Сиб является отдельной вилайей, но входит в мухафазу Маскат, поэтому при сравнении с другой страной читайте обе строки Маската вместе.',
    citiesCol: 'Районы',
    ...
    citeHeading: 'Как ссылаться на индекс',
    citeIntro: 'Цитировать эту страницу и ставить на неё ссылку можно свободно. Пожалуйста, указывайте вместе с цифрой размер выборки и дату и уточняйте, что речь идёт о фрихолд-предложении, доступном иностранным покупателям, а не обо всём рынке Омана.',
    citeLabel: 'Готовая ссылка для цитирования',
    citeCopy: 'Скопировать',
    citeCopied: 'Скопировано',
    citeDataLabel: 'Машиночитаемая версия',
    citeDataNote: 'Те же цифры в формате JSON, обновляются вместе со страницей. Подходит для графиков, таблиц и исследований.',
    ...
    '@graph': [
      {
        ...
        // A Dataset with no distribution is a claim, not a dataset. The feed is
        // language independent, so every locale points at the same URL.
        distribution: [
          {
            '@type': 'DataDownload',
            encodingFormat: 'application/json',
            contentUrl: `${SITE}/api/price-index.json`,
            name: 'Oman Property Price Index, JSON',
          },
        ],
      },
      // The ranked community table, marked up so the ordering itself is
      // machine readable rather than just visible.
      ...(index?.byArea?.length
        ? [{
          '@type': 'ItemList',
          '@id': `${url}#communities`,
          ...
          numberOfItems: index.byArea.length,
          itemListElement: index.byArea.map((a, i) => ({
            '@type': 'ListItem',
            position: i + 1,
            name: a.label,
            ...(a.medianPpsm ? { description: `Median ${a.medianPpsm} OMR per m² over ${a.n} listed homes` } : {}),
            url: `${SITE}${lang === 'en' ? '' : `/${lang}`}/project?area=${encodeURIComponent(a.key)}`,
          })),
        }]
        : []),
      {

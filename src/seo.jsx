...
function resolve(pathname, lang = 'en') {
  ...
  // Project pages — localized title/desc templates around the project name.
  const m = pathname.match(/^\/buy\/([^/]+)\/?$/)
  if (m) {
    const name = titleCase(decodeURIComponent(m[1]))
    const t = {
      en: `${name} — Property for Sale in Oman | Irfan Investment Group`,
      ru: `${name} — недвижимость на продажу в Омане | Irfan Investment`,
      ar: `${name} — عقار للبيع في عُمان | Irfan Investment Group`,
      fa: `${name} — ملک برای فروش در عمان | Irfan Investment Group`,
    }
    const d = {
      en: `Explore ${name}, a curated development in Oman offered by Irfan Investment Group — view available units, pricing and details.`,
      ru: `${name} — проект в Омане от Irfan Investment Group: доступные юниты, цены и детали.`,
      ar: `اكتشف ${name}، مشروعاً مختاراً في عُمان من Irfan Investment Group — الوحدات المتاحة والأسعار والتفاصيل.`,
      fa: `${name}، پروژه‌ای منتخب در عمان از Irfan Investment Group — واحدهای موجود، قیمت و جزئیات.`,
    }
    return { title: pick(t, lang), desc: pick(d, lang), index: true }
  }
